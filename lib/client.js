 ...
 *
 * One bound scope (`ctx.settingsScope.bind`) is the only source either half
  ...
  factory: (require) => {
    ...
    function createCard(scope) {
      ...
      return function LegionCard() {
        ...
        const setField = (key, value) => write(() => scope.set(key, value))
        ...
        return createElement(
          'li',
          { className: `lg-card${open ? ' lg-card-open' : ''}` },
          createElement(
            'button',
            {
              type: 'button',
              className: 'lg-header',
              'aria-expanded': open,
              'aria-label': `${open ? 'Collapse' : 'Expand'}: Legion v${VERSION}`,
              onClick: () => { setOpen(!open) },
            },
            createElement(
              'span',
              { className: 'lg-head' },
              createElement('span', { className: 'lg-name' }, `Legion v${VERSION}`),
              createElement(
                'span',
                { className: 'lg-desc' },
                `Split into ${section.minSubtasks}-${section.maxSubtasks} subtasks, depth ≤ ${section.maxDepth}, `
                + `${section.workersPerTask} worker(s)/leaf · ${strategy.label} merge · `
                + `approval ${approval ? 'required' : 'off'} · budget ${section.maxTasksPerRun === 0 ? '∞' : section.maxTasksPerRun}`,
              ),
            ),
            createElement('span', {
              className: open ? 'lg-chevron lg-chevron-open' : 'lg-chevron',
              'aria-hidden': true,
            }),
          ),
          open
    ...
    function apply(ctx) {
      ...
      // registration does not depend on plugin load order. The card closes
      // over its own bound scope, so the entry takes no injected props.
